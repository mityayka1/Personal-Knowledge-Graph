...
#### Issue #61: Carousel UX для событий ✅ COMPLETED
...
**Tasks:** ✅ COMPLETED (PR #63)
- [x] Реализовать Carousel state в Redis (текущий индекс, список eventIds)
- [x] Обновлять сообщение через `editMessageText` при навигации
...
### Улучшения UX Phase A (v2)

#### A2.1: Архитектура Bot + Userbot

**Важное разделение:** Бот показывает UI, юзербот отправляет сообщения.
...
**Endpoint в Telegram Adapter:**
```typescript
// POST /telegram/send-as-user
interface SendAsUserRequest {
  chatId: string;      // Telegram chat ID
  text: string;        // Message text
  replyToMsgId?: number; // Optional reply
}
```

#### A2.2: Улучшенный Approval Flow

**Три кнопки вместо двух:**
...
#### A2.3: Двухрежимное редактирование

При нажатии "Изменить" показываем выбор:

```
Как хочешь изменить сообщение?

[💡 Задать] — опиши что написать, AI сформулирует
[📝 Как есть] — напиши точный текст для отправки
```

**Flow "Задать":**
```
👤 User: [нажимает 💡 Задать]
🤖 Bot: Опиши, что хочешь написать:
👤 User: спросить про документы, мягко напомнить
🤖 Bot: 📤 Отправить сообщение?

         Текст:
         Маш, привет! Как там с документами?
         Буду ждать, когда будет удобно 🙏
...
**Flow "Как есть":**
```
👤 User: [нажимает 📝 Как есть]
🤖 Bot: Напиши сообщение для отправки:
👤 User: Маш, не давлю, но очень переживаю. Когда будет готово?
🤖 Bot: 📤 Отправить сообщение?

         Текст:
         Маш, не давлю, но очень переживаю. Когда будет готово?

         [✅ Отправить] [✏️ Изменить] [❌ Отмена]
```

#### A2.4: Проактивные кнопки действий

**Morning Brief с action buttons:**
...
⚠️ Просроченные:
• Отправить отчёт Ивану (просрочено 2 дня)
  [💬 Написать Ивану]
```
...
**При нажатии `[💬 Написать Маше]`:**
```
🤖 Bot: 📤 Написать Маше про документы?

         Текст:
         Маш, привет! Как там с документами?
         Напомню, что жду от тебя.

         [✅ Отправить] [✏️ Изменить] [❌ Отмена]
```

#### A2.5: Follow-up после отправки

После успешной отправки предлагаем создать напоминание:
...
3. **Services:**
   - TelegramSendService — отправка сообщений через юзербот

4. **API:**
   - POST /agent/act — выполнение действий
   - POST /telegram/send-as-user — отправка от имени пользователя

5. **Telegram Bot:**
   - /act команда
   - Approval callbacks (approve, reject, edit)
   - Edit mode selection (describe/verbatim)
   - Proactive action buttons в Morning Brief
   - Follow-up suggestion после отправки
