...
Entities находятся в `packages/entities/src/`.

**Регистрация entities:** Единственный источник правды — `apps/pkg-core/src/database/entities.ts`. При добавлении новой entity добавляй её ТОЛЬКО туда. Этот файл импортируется в:
- `data-source.ts` — для TypeORM CLI (миграции)
- `database.config.ts` — для NestJS runtime
