 ...
 * - ✅ Новый → Use the new fact
 * - ❌ Старый → Keep the existing fact
 * - 🔀 Оба → Keep both facts
 */
@Injectable()
export class FactCallbackHandler {
    ...
    try {
      ...
      if (!result.success) {
        ...
        await ctx.editMessageText(
          `❌ Ошибка: ${result.error || 'Не удалось обработать'}`,
          { parse_mode: 'HTML' },
        );
        return;
      }
  ...
  private getResolutionFeedback(resolution: FactResolution, action?: string): string {
    switch (resolution) {
      case 'new':
        return `✅ <b>Принят новый факт</b>\n\nСтарый факт помечен как устаревший.`;
      case 'old':
        return `✅ <b>Сохранён старый факт</b>\n\nНовый факт отклонён.`;
      case 'both':
        return `✅ <b>Сохранены оба факта</b>\n\nОба факта сосуществуют в базе.`;
      default:
        return `✅ <b>Конфликт разрешён</b>`;
    }
  }
  ...
  private getShortFeedback(resolution: FactResolution): string {
    switch (resolution) {
      case 'new':
        return '✅ Новый факт принят';
      case 'old':
        return '✅ Старый факт сохранён';
      case 'both':
        return '✅ Оба факта сохранены';
      default:
        return '✅ Готово';
    }
  }
