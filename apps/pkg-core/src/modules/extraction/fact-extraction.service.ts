...
export class FactExtractionService {
  ...
  private buildCompactPrompt(
    ...
    return `Извлеки факты о ${name}.

## Правила
ФАКТ = стабильный атрибут сущности, сохраняющийся долгое время.
...
Типы: position, company, department, specialization, skill, education, role, birthday, location, family, hobby, language, health, status, communication, preference, inn, legal_address.
...
Текст: ${cleanText}`;
  ...
  private buildBatchPrompt(
    ...
    return `Извлеки факты о ${name}. Удали дубликаты.
