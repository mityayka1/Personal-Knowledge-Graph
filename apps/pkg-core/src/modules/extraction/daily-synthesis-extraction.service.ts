...
2. **ЗАДАЧИ** — конкретные действия, TODO, что нужно сделать
   ...
   **НЕ извлекай как задачу если:**
   ...
   - Глагол в прошедшем времени: "настроил CI/CD", "написал отчёт" — это отчёт, НЕ задача
   - Только БУДУЩИЕ задачи: "настроить", "написать", "проверить"
...
3. **ОБЯЗАТЕЛЬСТВА** — обещания, договорённости, напоминания
   ...
   **КРИТЕРИИ КАЧЕСТВА ОБЯЗАТЕЛЬСТВ — НЕ извлекай если:**
   ...
   - Это завершённое действие: "обсудили", "согласовали", "отправил", "настроил"
   - Это информирование: "сообщил", "показал", "написал результат"
   - Это подтверждение: "понял", "принял", "учёл"
   - Только БУДУЩИЕ обещания: "сделаю", "отправлю завтра", "подготовлю к пятнице"

5. **ФАКТЫ** — ТОЛЬКО стабильные атрибуты:
   ❌ Временные: "болеет", "в отпуске", "устал", "в командировке"
   ❌ Проектные данные: стоимость работ, бюджет, тех. параметры (API, серверы, конфиги)
   ✅ Стабильные: должность, компания, навык, день рождения, образование
...
1. confidence 0.0-1.0 = ACTIONABILITY (можно ли реально действовать по этому элементу?):
