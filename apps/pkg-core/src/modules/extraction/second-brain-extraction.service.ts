...
6. НЕ извлекай как promise_by_me / promise_by_them:
   ❌ Завершённые действия: "обсудили", "согласовали", "отправил", "настроил"
   ❌ Информирование: "сообщил", "показал", "написал результат"
   ❌ Подтверждения: "понял", "принял", "учёл"
   ✅ Только БУДУЩИЕ actionable обещания: "сделаю", "отправлю завтра", "подготовлю к пятнице"

7. НЕ извлекай как task:
   ❌ Завершённые действия: "настроил CI/CD", "написал отчёт", "проверил код"
   ✅ Только БУДУЩИЕ задачи: "настроить CI/CD", "написать отчёт", "проверить код"
   Правило: глагол в прошедшем времени (-л, -ла, -ли) = НЕ задача

8. Факты — ТОЛЬКО стабильные атрибуты:
   ❌ Временные: "болеет", "в отпуске", "устал", "в командировке"
   ❌ Проектные данные: стоимость работ, бюджет, тех. параметры (API, серверы, конфиги)
   ❌ Финансовые операции: "оплатил 50 000", "выставил счёт"
   ✅ Стабильные: должность, компания, навык, день рождения, образование, постоянное место жительства

9. Используй СУЩЕСТВУЮЩИЕ АКТИВНОСТИ для привязки:
...
ВАЖНО:
- confidence 0.0-1.0 = ACTIONABILITY (можно ли реально действовать по этому элементу?):
  0.9+ = явно сказано, конкретное действие с деталями и сроками
  0.7-0.9 = подразумевается из контекста, но actionable
  <0.7 = нечёткое, возможно неточная интерпретация — лучше НЕ извлекать
