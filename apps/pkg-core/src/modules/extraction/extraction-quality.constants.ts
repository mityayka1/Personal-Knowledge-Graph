...
export const INFORMATIONAL_COMMITMENT_PATTERNS: RegExp[] = [
  ...
  /(?<!\p{L})(завершил|закрыл|решил|исправил|доделал|переделал|сделал|проверил|загрузил|выполнил|установил|подключил|разобрал|создал|удалил|реализовал|перенёс)(а|и|ся|ась|ись|ось)?(?!\p{L})/iu,
  // Passive past participles: создан, внесён, выполнен, отправлен, etc.
  /(?<!\p{L})(создан|внесён|внесен|внесены|выполнен|отправлен|настроен|обновлён|обновлен|удалён|удален|завершён|завершен|исправлен|установлен|подключен|загружен|реализован)(а|о|ы)?(?!\p{L})/iu,
