...
export const INFORMATIONAL_COMMITMENT_PATTERNS: RegExp[] = [
  ...
  /(?<!\p{L})(обсуди|согласова|подтверди|отправи|переда|уточни|рассмотре|проанализирова|обнови|настрои|подготови|добави|измени|замени|перемести|опубликова|обработа)(л[аоие]?с[ья]|л[аоие]?)(?!\p{L})/iu,
  // Already past-tense forms (stem already ends in -л): завершил, проверил, сделал, создал, etc.
  /(?<!\p{L})(завершил|закрыл|решил|исправил|доделал|переделал|сделал|проверил|загрузил|выполнил|установил|подключил|разобрал|создал|удалил|реализовал|перенёс|поменял)(а|и|ся|ась|ись|ось)?(?!\p{L})/iu,
  // Passive past participles: создан, внесён, выполнен, отправлен, добавлен, etc.
  /(?<!\p{L})(создан|внесён|внесен|внесены|выполнен|отправлен|настроен|обновлён|обновлен|удалён|удален|завершён|завершен|исправлен|установлен|подключен|загружен|реализован|добавлен|изменён|изменен|заменён|заменен|перемещён|перемещен|опубликован|подготовлен|согласован|передан|проверен)(а|о|ы)?(?!\p{L})/iu,
...
const PAST_TENSE_TASK_PATTERNS: RegExp[] = [
  ...
  /^\s*(обсуди|отправи|настрои|подготови|написа|обнови|проанализирова|согласова|переда|оптимизирова|добави|измени|замени|перемести|опубликова|обработа)(л[аоие]?с[ья]|л[аоие]?)(?!\p{L})/iu,
  /^\s*(завершил|проверил|создал|удалил|исправил|подключил|загрузил|протестировал|сделал|разобрал|выполнил|закрыл|перенёс|установил|доработал|переработал|реализовал|поменял)(а|и|ся|ась|ись|ось)?(?!\p{L})/iu,
  // Also catch "был + participle" constructions
  /^\s*был[аоие]?\s+(завершен|выполнен|отправлен|настроен|создан|удалён|обновлён|добавлен|изменён|подготовлен|согласован)/iu,
  // Standalone passive participles at title start: "Добавлены артикулы", "Создан отчёт"
  /^\s*(создан|внесён|внесен|внесены|выполнен|отправлен|настроен|обновлён|обновлен|удалён|удален|завершён|завершен|исправлен|установлен|подключен|загружен|реализован|добавлен|изменён|изменен|заменён|заменен|перемещён|перемещен|опубликован|подготовлен|согласован|передан|проверен)(а|о|ы)?(?!\p{L})/iu,
];
