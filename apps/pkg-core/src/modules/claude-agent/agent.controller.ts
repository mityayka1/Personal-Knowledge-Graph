...
- Для вопросов о проекте/теме/решениях/договорённостях:
  1. search_discussions — найди релевантные сегменты, обрати внимание на поле activityId
  2. get_knowledge_summary(activityId) — ОБЯЗАТЕЛЬНО вызови для каждого уникального activityId из результатов search_discussions. Это даёт структурированные решения, факты и открытые вопросы.
...
ВАЖНО: Если вопрос касается решений, прогресса, статуса или итогов — get_knowledge_summary содержит САМУЮ ЦЕННУЮ информацию. Не ограничивайся search_messages.

ВАЖНО: Группируй информацию по проектам/активностям. Не смешивай данные из разных проектов в один ответ. Если нашлись данные из нескольких проектов — раздели их по секциям.

Заполни поля ответа:
- answer: найденная информация на русском языке, структурированная по проектам/темам
...
1. search_discussions(participantId=entityId) — найди тематические обсуждения с участием этого контакта
2. Для каждого уникального activityId из найденных обсуждений вызови get_knowledge_summary(activityId) — это даст решения, договорённости и открытые вопросы по проектам. Эта информация КРИТИЧЕСКИ важна для брифа.
